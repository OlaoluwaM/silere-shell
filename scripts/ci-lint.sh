...
# Literal widget colours bypass the shared palette and cannot participate in theme
# changes. Theme.qml owns literal definitions; every visual component consumes a token.
...
if [ -n "$widget_hex" ]; then
  ...
  ok "theme tokens" "widgets contain no literal hex colours"
fi
...
if [ -f "$theme_tmpl" ] && [ -f "$theme_loader" ]; then
    ...
    # transitioning describe load/animation state, not colours, so they have no
    # template key to cover.
    used=$(grep -rhoE 'MatugenTheme\.[a-zA-Z_][a-zA-Z0-9_]*' --include='*.qml' . \
           | sed 's/^MatugenTheme\.//' \
           | grep -vE '^(_|qml$|usingFallback$|paletteStale$|transitioning$)' | sort -u)
    theme_gap=0
