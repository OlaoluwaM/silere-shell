...
probe_root="$(mktemp -d "${TMPDIR:-/tmp}/silere-upstream-connectivity.XXXXXX")"
probe_pid=""
cleanup() {
...
_probe_project "$ROOT" scripts/probe-upstream-connectivity.qml "$probe_root/project"
...
XDG_CONFIG_HOME="$probe_root/config" XDG_STATE_HOME="$probe_root/config" \
    ...
    qs -p "$probe_root/project/probe-upstream-connectivity.qml" --no-color >"$probe_root/probe.log" 2>&1 &
probe_pid=$!
...
if ! grep -q 'PROBE-UPSTREAM-CONNECTIVITY passed' "$probe_root/probe.log" \
        || grep -q 'PROBE-FAIL' "$probe_root/probe.log" || [ -n "$errs" ]; then
    cat "$probe_root/probe.log" >&2
    echo 'FAIL: connectivity probe failed or logged a runtime error' >&2
    exit 1
fi
grep -oE 'PROBE-UPSTREAM-CONNECTIVITY passed [0-9]+ checks' "$probe_root/probe.log" | tail -1
