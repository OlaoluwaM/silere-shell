...
QtObject {
    ...
    property Connections _eventConn: Connections {
        ...
        function onRawEvent(event) {
            ...
            // a reload can change gaps_out, and that moves every tiled edge
            if (n === "configreloaded") root._queryWindowGap()
            root._layoutTick++
        }
    }

    // hyprctl reports either an int or a CSS-style side list. A bar has one horizontal
    // inset, so asymmetric left and right values collapse to the larger side.
    ...
    // retain one request that lands during an in-flight query, so config edits cannot
    // leave the reported gap stale.
    function _queryWindowGap(): void {
        if (!SystemTools.hasHyprctl) return
        if (_gapProc.running) {
            root._gapRecheckPending = true
            return
        }
        _gapProc.exec(["hyprctl", "getoption", "general:gaps_out", "-j"])
    }

    property BoundedProcess _gapProcess: BoundedProcess {
        id: _gapProc
        timeoutMs: 5000
        ...
        onExited: (code) => {
            ...
            if (code === 0 && !_gapProc.timedOut) {
                ...
                // A transient query failure must not make the bar jump from its last
                // confirmed compositor gap back to the configured width fraction.
    ...
    // SystemTools discovers hyprctl asynchronously, after this adapter is built.
    property Connections _systemToolsConn: Connections {
        target: SystemTools
        function onScanRevisionChanged() { root._queryWindowGap() }
    }
}
